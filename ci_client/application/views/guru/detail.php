<div class="container pt-5">
    <h3><?= $title ?></h3>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb bg-light">
            <li class="breadcrumb-item"><a>Guru</a></li>
            <li class="breadcrumb-item "><a href="<?= base_url('guru'); ?>">List Data</a></li>
            <li class="breadcrumb-item active" aria-current="page">Detail Data</li>
        </ol>
    </nav>
    <div class="row mt-3">
        <div class="col-md-6">

            <div class="card">
                <div class="card-header bg-success">
                    Detail Data Guru
                </div>
                <div class="card-body">
                    <h5 class="card-title"><b>ID GURU :</b><br><?= $data_guru['id_guru']?></h5>
                    <p class="card-text"><b>Nama :</b><br><?= $data_guru['nama_guru']?></p>
                    <p class="card-text"><b>Pendidikan :</b><br><?= $data_guru['pendidikan']?></p>
                    <p class="card-text"><b>Status :</b><br><?= $data_guru['status']?></p>

                    <p></p>
                    <a href="<?= base_url(); ?>guru" class="btn btn-secondary">Kembali</a>
                </div>
            </div>

        </div>
    </div>
</div>
<br>